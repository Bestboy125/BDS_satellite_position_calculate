...
void readeph(parameter s[1329],string broadcastfile)
{
	...
	//进入读文件体程序
	for (realnumber; realnumber < 81935; )
	{
		getline(file, buf);//每一次取一行的内容
		realnumber++;//实际的总行数加一
		if (realnumber < 74568)
			...
			number = (realnumber - 74567) % 8;//开始读文件体的小循环，因为一个结构有8行，这个用于判断到了哪一行，到了70463之后就开始是C开头的卫星了
		//先判断开头，只读入C和G开头的卫星
		...
		case (2)://余数是2，第二行的内容
		{
			...
			//开始赋值
			s[order].AODE = stod(IODE) * pow(10, stod(IODE_power));
		...
		case (5)://余数是5，第五行的内容
		{
			...
			s[order].omega = stod(omega1) * pow(10, stod(omega1_power));
		...
		case (6)://余数是6，第六行的内容
		{
			...
			string wn;//BDS周数（与TOE一同表示时间）主体
			string wn_power;//BDS周数（与TOE一同表示时间）幂
			...
			s[order].spare1 = stod(l2_code) * pow(10, stod(l2_code_power));
			s[order].wn = stod(wn) * pow(10, stod(wn_power));
			s[order].spare2 = stod(l2_P_tag) * pow(10, stod(l2_P_tag_power));
			break;
		}
		case (7)://余数是7，第七行的内容
		{
			...
			s[order].TGD1 = stod(TGD) * pow(10, stod(TGD_power));
			s[order].TGD2 = stod(IODC) * pow(10, stod(IODC_power));
			break;
		}
		case (0)://余数是0，第八行的内容
		{
			//第八行的参数
			string SendTime;//电文发送时刻（单位为BDS周的秒，通过交接字（HOW）中的Z计数得出）主体
			string SendTime_power;//电文发送时刻（单位为BDS周的秒，通过交接字（HOW）中的Z计数得出）幂
	...
	for (int i = 0; i < 1329; i++)
	{
		...
		cout << "该卫星在读取的BDS卫星和北斗卫星中的序列号为：" << s[i].broadcastnum << endl;
